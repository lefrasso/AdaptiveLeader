...
import { CommunicationEvaluator } from "@/components/assessments/communication-evaluator";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "comm" });
  return { title: t("meta.title"), description: t("meta.description") };
}

export default async function CommunicationEvaluatorPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("comm");

  return (
    <div className="flex min-h-full flex-col">
      <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-4 px-5 py-3 print:hidden">
        <Link
          href="/"
      ...
      <footer
        ...
        style={HERO_STYLE}
      >
        <div className="mx-auto w-full max-w-4xl px-5 py-6 text-[13px] text-ice">
          {t("footer")}
        </div>
      </footer>
    </div>
